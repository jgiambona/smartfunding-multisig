 ...
 * @dev A 2/3 multisig contract compatible with Trezor or Ledger-signed messages.
 * 
...
contract MultiSig2of3 {
  ...
  /**
   * @dev The 3 addresses which control the funds in this contract.  The
  ...
  constructor(address owner1, address owner2, address owner3) public {
    require(owner1 != address(0), "1");
    require(owner2 != address(0), "1");
    require(owner3 != address(0), "1");
  ...
  /**
   * @dev The fallback function for this contract.
   */
  ...
  /**
   * @dev Generates the message to sign given the output destination address and amount.
  ...
  /**
   * @dev Send the given amount of ETH to the given destination using
   * the two triplets (v1, r1, s1) and (v2, r2, s2) as signatures.
   * v1 and v2 should be 0x00 or 0x01 corresponding to 0x1b and 0x1c respectively.
   */
    ...
    private view returns (bool)
  {
    ...
    address addr1 = ecrecover(message, v1.add(27), r1, s1);
    address addr2 = ecrecover(message, v2.add(27), r2, s2);
  ...
  /**
   * @dev Generate the the unsigned message (in bytes32) that each owner's
  ...
  /**
   * @dev Confirm the pair of addresses as two distinct owners of this contract.
   */
  function _distinctOwners(address addr1,
                           address addr2)
    private view returns (bool)
  {
  ...
  /**
   * @dev Construct the byte representation of the ascii-encoded
  ...
  function _hashToAscii(bytes32 hash) private pure returns (bytes) {
    ...
    for (uint i = 0; i < 32; i++) {
      byte b = hash[i];
      byte hi = byte(uint8(b).div(16));
      byte lo = byte(uint8(b).sub(uint8(hi).mul(16)));
      
      s[i.mul(2)] = _char(hi);
      s[i.mul(2).add(1)] = _char(lo);
    }
    return s;
  }

  /**
   * @dev Convert from byte to ASCII of 0-f
   * http://www.unicode.org/charts/PDF/U0000.pdf
   */
  function _char(byte b) private pure returns (byte c) {
    if (b < 10) {
      return byte(uint8(b).add(0x30));
    } else {
      return byte(uint8(b).add(0x57));
    }
  }
}
